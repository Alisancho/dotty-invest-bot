...
trait Absorption {
  def absorptionUp(l: HistoricalCandles)(instrument:Instrument)(f: String => Task[_])(schedulerDB: SchedulerService): Task[_] =
    for {
  ...
  def absorptionDown(l: HistoricalCandles)(instrument:Instrument)(f: String => Task[_])(schedulerDB: SchedulerService): Task[_] =
    for {
      k  <- Task { l.candles.asScala.toList }
      q2 = k(k.size - 2)
      q1 = k.last
